...
@author: Yogesh Kumar (forked and modified from Chenhao Zhang github repo)

Example:  python eye.py image_dir/
"""
import sys, os
...
annotationInfo_1 = ['top left', 'bottom right', 'left eye', 'right eye', 'nose', 'left mouth', 'right mouth']
for a_file in files:
    ...
    for i in range(7):
        if i%2 == 0:
            row_frame = tk.Frame(toolbox)
            row_frame.pack(side=tk.TOP)

        btn = tk.Button(row_frame, text=annotationInfo_1[i], relief=tk.GROOVE, width=10).pack(side=tk.LEFT)
...
#        reDraw(point_now)
    ...
    def btn8_c():
        ...
        for i in range(N_points):
         ...
         #top.destroy()
    btn8 = tk.Button(toolbox, text="Clear All Points(CA)", command=btn8_c)
    Buttons.append(btn8) 
